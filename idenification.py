...
train_filenames = ['dataset.tfrecords']
...
tfe = tf.compat.v1
train_writer = tf.summary.create_file_writer(os.path.join(checkpoint_dir, "train"))
val_writer = tf.summary.create_file_writer(os.path.join(checkpoint_dir, "val"))

# define learning_rate and beta1 tensors for cyclical policy
learning_rate_tf = tf.Variable(base_lr)
beta1_tf = tf.Variable(max_beta1)

global_step = tfe.train.get_or_create_global_step()

# AdamOptimizer with cyclical policy
optimizer = tfe.train.AdamOptimizer(learning_rate=learning_rate_tf,
                                   beta1=beta1_tf, beta2=0.99)

checkpoint_prefix = os.path.join(checkpoint_dir, "ckpt")

root = tfe.train.Checkpoint(optimizer=optimizer,
                      model=model,
                      optimizer_step=tfe.train.get_or_create_global_step())
...
try:
    for (batch, (Xi, Xj, train_labels)) in enumerate(train_dataset):
        ...
        with train_writer.as_default(), tf.summary.record_summaries_every_n_global_steps(300):
            ...
            with tf.GradientTape() as tape:
                ...
                tf.summary.scalar('loss', train_loss_np)
                tf.summary.scalar('learning_rate', learning_rate_tf)
                tf.summary.scalar('beta1', beta1_tf)
        ...
        if global_step.numpy() % FLAGS.eval_model_every_n_steps == 0:
            ...
            with val_writer.as_default(), tf.contrib.summary.always_record_summaries():
                tf.summary.scalar('loss', mean_val_loss)
                tf.summary.scalar('mean_similarity', mean_similarity_np)
                tf.summary.scalar('mean_dissimilarity', mean_dissimilarity_np)
                tf.summary.scalar('embedding_mean_distance', np.abs(mean_similarity_np - mean_dissimilarity_np))
except Exception:
    transform_more(train_dataset)